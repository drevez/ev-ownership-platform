...
export function VehicleImage({ src, alt, vehicleId, displayName }: VehicleImageProps) {
    ...
    return (
      <div className="w-full h-full bg-gradient-to-br from-slate-800 via-slate-700 to-slate-900 flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-6">
          ...
          <h3 className="text-lg font-semibold text-white/80 mb-1">Image Coming Soon</h3>
          {displayName && (
            <p className="text-sm text-white/60">{displayName}</p>
          )}
          {vehicleId && (
            <p className="text-xs text-white/40 mt-2 font-mono">{vehicleId}</p>
          )}
        </div>
      </div>
    )
